...
enum APP_TYPE {
  ...
  APP_TYPE_CEF = 13,
  APP_TYPE_UNI_APP = 14,
  APP_TYPE_LINUX_CPP = 16,
  APP_TYPE_LINUX_GO=17,
  APP_TYPE_LINUX_PYTHON=18,
  APP_TYPE_LINUX_JAVA=19,
};
...
class IRtcEngineInternal : public IRtcEngineEx, public agora::base::IParameterEngine {
 protected:
 ...
 public:
  ...
  virtual int getParametersEx(const RtcConnection& connection, const char* key,
  ...
  virtual agora_refptr<IRtcConnection> getConnection(
      const RtcConnection& connection = {}) const = 0;
 ...
 public:
  EncryptionConfigInternal(const EncryptionConfig& config)
      : encryptionMode(config.encryptionMode),
        encryptionKey(config.encryptionKey ? config.encryptionKey : ""),
        datastreamEncryptionEnabled(config.datastreamEncryptionEnabled) {
