...
#include "api2/AgoraAtomicOps.h"
#include <api/aosl_mpq.h>
#include <api/cpp/aosl_ref_class.h>

#ifndef __AGORA_REF_COUNTED_OBJECT_INTERNAL_H__
#define __AGORA_REF_COUNTED_OBJECT_INTERNAL_H__
#endif

#if defined(__AGORA_REF_COUNTED_OBJECT_H__)
#error AgoraRefCountedObject is deprected now, its only purpose is for API compatiable. Consider using <api2/internal/AgoraRefCountedObjectInternal.h> instead.
#endif
...
class RefObjectDeleter
{
  public:
  template<class T>
  void operator()(const T* p) const {
      if(aosl_ref_class::run(aosl_mpq_main(), AOSL_REF_INVALID, __FUNCTION__, [p](const aosl_ts_t &queued_ts, aosl_refobj_t robj){
        delete p;
      }) < 0) {
        /*
        * main thread may not exist any more, delete in current thread context anyway.
        */
        delete p;
      }
  }
...
template <class T, class Deleter = RefObjectDeleter>
